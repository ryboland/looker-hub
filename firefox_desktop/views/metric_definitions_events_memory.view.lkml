...
base.windows_ubr,
                ...
                metrics.DATE(submission_date) AS analysis_basis,
                {% elsif aggregate_metrics_by._parameter_value == 'week'  %}
                (FORMAT_DATE(
                    '%F',
                    DATE_TRUNC(metrics.DATE(submission_date),
                    WEEK(MONDAY)))
                ) AS analysis_basis,
                {% elsif aggregate_metrics_by._parameter_value == 'month'  %}
                (FORMAT_DATE(
                    '%Y-%m',
                    metrics.DATE(submission_date))
                ) AS analysis_basis,
                ...
                (EXTRACT(
                    YEAR FROM metrics.DATE(submission_date))
                ) AS analysis_basis,
                {% else %}
                NULL as analysis_basis,
                {% endif %}
                metrics.DATE(submission_date) AS submission_date
            FROM
                (
    SELECT
        *
    FROM
        ...
        WHERE
            ...
            ON
                base.submission_date = metrics.DATE(submission_date) AND
