...
explore: creditcards_sync {
  ...
  always_filter: {
    filters: [
      channel: "moz-fx-data-shared-prod.fenix.creditcards^_sync",
  ...
  join: creditcards_sync__events__extra {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${creditcards_sync.events__extra}) AS creditcards_sync__events__extra ;;
  }

  join: creditcards_sync__metrics__jwe {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${creditcards_sync.metrics__jwe}) AS creditcards_sync__metrics__jwe ;;
  }

  join: creditcards_sync__metrics__labeled_rate {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${creditcards_sync.metrics__labeled_rate}) AS creditcards_sync__metrics__labeled_rate ;;
  }

  join: creditcards_sync__metrics__labeled_rate__value {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${creditcards_sync.metrics__labeled_rate__value}) AS creditcards_sync__metrics__labeled_rate__value ;;
  }
  ...
  join: creditcards_sync__metrics__text {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${creditcards_sync.metrics__text}) AS creditcards_sync__metrics__text ;;
  }

  join: creditcards_sync__metrics__url {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${creditcards_sync.metrics__url}) AS creditcards_sync__metrics__url ;;
  }
