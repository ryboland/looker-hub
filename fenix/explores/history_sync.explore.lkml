...
explore: history_sync {
  ...
  join: history_sync__events__extra {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__events.events__extra}) AS history_sync__events__extra ;;
  }

  join: history_sync__metrics__jwe {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__metrics.metrics__jwe}) AS history_sync__metrics__jwe ;;
  }

  join: history_sync__metrics__labeled_rate {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__metrics.metrics__labeled_rate}) AS history_sync__metrics__labeled_rate ;;
  }

  join: history_sync__metrics__labeled_rate__value {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__metrics__labeled_rate.metrics__labeled_rate__value}) AS history_sync__metrics__labeled_rate__value ;;
  }

  join: history_sync__metrics__labeled_string__history_sync_failure_reason {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__metrics__labeled_string.metrics__labeled_string__history_sync_failure_reason}) AS history_sync__metrics__labeled_string__history_sync_failure_reason ;;
  }

  join: history_sync__metrics__text {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__metrics.metrics__text}) AS history_sync__metrics__text ;;
  }

  join: history_sync__metrics__url {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__metrics.metrics__url}) AS history_sync__metrics__url ;;
  }

  join: history_sync__ping_info__experiments {
    relationship: one_to_many
    sql: LEFT JOIN UNNEST(${history_sync__ping_info.ping_info__experiments}) AS history_sync__ping_info__experiments ;;
  }
